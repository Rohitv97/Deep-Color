...
#Load weights
def color_xception():
    xception = Xception(weights=None, include_top=True)
    xception.load_weights('xception_weights.h5')
...
# Change to '/data/images/Test/' to use all the 500 test images
    ...
    color_me_embed = create_xception_embedding(color_me)
...
# Test model
    ...
    for i in range(len(output)):
        ...
        cur[:,:,0] = color_me[i][:,:,0]
        cur[:,:,1:] = output[i]
        imsave("static/xce/img_xception_"+str(i)+".png", lab2rgb(cur))
